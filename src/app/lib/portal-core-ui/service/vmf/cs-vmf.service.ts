...
export class CsVMFService {
  ...
  private getVMFFeature(url: string, polygon: string, apikey: string, maps: string): Observable<any> {
    ...
    const body=
    {
      "key": apikey, // e.g. APIKEY from https://api-docs.native-land.ca/
      "maps": maps, // e.g. "territories"
      "polygon_geojson": {
            ...
            if (this.cancelledLayers.indexOf(layer.id) === -1) {
              viewer.dataSources.add(dataSource).then((dataSrc: any) => {
                ...
                for (let i = 0; i < entities.length; i++) {
                  ...
                  const cesiumColor = Color.fromRgba(value);
                  ...
                  if (entity.polygon) {
                    ...
                    //Extrude the polygon
                    entity.polygon.extrudedHeight = new ConstantProperty(1.0);
                  }
                }
